...
resourcename="carparts"  # lower case, only
resourcegroup="carparts_rg"
...
# git clone https://github.com/apilogicserver/carparts.git
# cd carparts
...
az group create --name carparts_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group carparts_rg --sku S1 --is-linux
az container create --resource-group carparts_rg --name carparts --image apilogicserver/carparts:latest --dns-name-label carparts --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//carparts.westus.azurecontainer.io:5656'
